...
class TodoAcceptanceTest extends AnyFreeSpec {
  ...
  "get" - {
    "returns existing item" in {
      ...
      assert(post(s"/items/$id", Map("text" -> text)).statusCode == 200)
      assert(
        get(s"/items/$id").contentString == s"""{"id":"$id","text":"$text"}""")
    }
  ...
  "post" - {
    ...
    "validation is present" in {
      val r =
        post(s"/items/invalid-id", Map("text" -> Seq.fill(101)('A').mkString))
      assert(r.statusCode == 400)
      assert(
        // can't get this test to pass without using a long line...
        //scalastyle:off
        r.contentString == """{"errors":["id: must only contain alphanumeric characters or underscore","text: length is not less than or equal 100"]}""")
      //scalastyle:on
    }
  }
  ...
  "put" - {
    "updates existing item" in {
      ...
      assert(
        get(s"/items/$id").contentString == s"""{"id":"$id","text":"$text2"}""")
    }
  ...
  private def get(path: String) =
    service(RequestBuilder().url(s"http://local/api/v1$path").buildGet())
      .unsafeRunSync()

  private def post(path: String, content: Map[String, String]) =
    service(
      RequestBuilder()
        .url(s"http://local/api/v1$path")
        .buildPost(json(content))).unsafeRunSync()

  private def put(path: String, content: Map[String, String]) =
    service(
      RequestBuilder().url(s"http://local/api/v1$path").buildPut(json(content)))
      .unsafeRunSync()

  private def delete(path: String) =
    service(RequestBuilder().url(s"http://local/api/v1$path").buildDelete())
      .unsafeRunSync()
